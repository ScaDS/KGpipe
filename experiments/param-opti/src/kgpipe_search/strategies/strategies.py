...
def _parameter_neighbors(anchor: PipelineConfig, search_space: Dict[str, Dict[str, Any]]) -> List[PipelineConfig]:
    anchor_keys = task_keys_from_pipeline_config(anchor)
    neighbors: List[PipelineConfig] = []
    ...
    for task, task_key in zip(anchor.tasks, anchor_keys):
        ...
        for binding in profile.bindings:
            ...
            for value in domain:
                ...
                for current in profile.bindings:
                    chosen = value if current.parameter.name == param_name else current.value
                    new_bindings.append(ParameterBinding(parameter=current.parameter, value=chosen))
                    name_parts.append(f"{current.parameter.name}={chosen}")
                ...
                neighbors.append(PipelineConfig(tasks=list(anchor.tasks), config_catalog=new_catalog))
...
def _config_distance(left: PipelineConfig, right: PipelineConfig, search_space: Dict[str, Dict[str, Any]]) -> float:
    if pipeline_config_snapshot_key(left, search_space) == pipeline_config_snapshot_key(right, search_space):
        return 0.0
...
def run_random(
    *,
    ...
    for _ in range(budget):
        candidate = sample_unevaluated_config(draw, search_space, pipeline_layout, evaluated_keys)
        key = pipeline_config_snapshot_key(candidate, search_space)
        score = evaluate_fn(candidate)
        history.append((score, candidate))
        evaluated_keys.add(key)
...
def run_qgns(
    *,
    ...
    """
    Quality-Guided Neighborhood Search (QGNS) from the paper.

    Maintains top-k anchors; with probability rho explores globally, otherwise samples
    one-step neighbors of an anchor.
    """
    if budget <= 0:
        return SearchRun(strategy="qgns", history=[], budget=0, decisions=[])
    ...
    if init_budget > 0:
        if init_strategy == "implementation_aware":
            init_set = implementation_aware_initialization(
                search_space, pipeline_layout, budget=min(init_budget, budget), y=y, rng=draw
            )
        else:
            init_set = random_initialization(
                search_space, pipeline_layout, budget=min(init_budget, budget), rng=draw
            )
        for cfg in init_set:
            key = pipeline_config_snapshot_key(cfg, search_space)
            if key in evaluated_keys:
    ...
    while len(history) < budget:
        if not history or draw.random() < rho:
            ...
            # Try anchors until we find an unevaluated neighbor; else fallback to explore.
            shuffled = list(anchors)
            draw.shuffle(shuffled)
            for anchor_score, anchor_cfg in shuffled:
                neighborhood = neighbors_at_distance_one(anchor_cfg, search_space, pipeline_layout, draw)
                unevaluated = [
                    n for n in neighborhood if pipeline_config_snapshot_key(n, search_space) not in evaluated_keys
                ]
                if not unevaluated:
                    continue
                candidate = draw.choice(unevaluated)
...
def run_hnr(
    *,
    ...
    """
    Heuristic Neighborhood Refinement (HNR) from the paper.

    Starts from an initial evaluated set, then iterates task-wise. For each task index,
    prefers trying implementation replacements, then parameter modifications, with occasional
    global exploration.
    """
    if budget <= 0:
        return SearchRun(strategy="hnr", history=[], budget=0, decisions=[])
    ...
    while len(history) < budget:
        ...
        for idx in range(len(best_cfg.tasks)):
            ...
            if draw.random() < rho:
                ...
                task_candidates = [
                    n for n in task_neighbors if pipeline_config_snapshot_key(n, search_space) not in evaluated_keys
                ]
                ...
                if task_candidates:
                    ...
                    param_candidates = [
                        n for n in param_neighbors if pipeline_config_snapshot_key(n, search_space) not in evaluated_keys
                    ]
                    if param_candidates:
                        candidate = draw.choice(param_candidates)
                        decision = f"param_neighbor(idx={idx})"
        ...
        # If we made no improvements during a full sweep, we still continue (paper keeps exploring).
        if not improved and len(history) < budget and draw.random() < rho:
            candidate = sample_unevaluated_config(draw, search_space, pipeline_layout, evaluated_keys)
            key = pipeline_config_snapshot_key(candidate, search_space)
            score = evaluate_fn(candidate)
...
def run_bayesian(
    *,
    ...
    """
    Simple Bayesian-optimization-like baseline using a distance-weighted surrogate,
    adapted to support explicit initialization strategies.
    """
    if budget <= 0:
        return SearchRun(strategy="bayesian", history=[], budget=0, decisions=[])
    ...
    while len(history) < budget:
        candidates: List[PipelineConfig] = []
        for _ in range(pool_size):
            candidates.append(sample_unevaluated_config(draw, search_space, pipeline_layout, evaluated_keys))
        ...
        history.append((score, best_candidate))
        evaluated_keys.add(key)
        decisions.append(f"acquisition(pred={best_pred:.4f},unc={best_unc:.4f},a={best_acq:.4f})")

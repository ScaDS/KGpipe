...
from kgpipe.common import Registry

def fusion_first_value_function(
    inputs: TaskInput, outputs: TaskOutput, config: ConfigurationProfile | None = None
):
    from param_opti.tasks.fusion_lib import fusion_first_value

    if config is not None:
        ontology_path = config.get_parameter_value("ontology_path")
    else:
    ...
    fusion_first_value(
        ...
        relation_matching_threshold=0.0,
        ontology_path=ontology_path,
    )

fusion_first_value_task = KgTask(
        name="fusion_first_value_task",
...
Registry.add_task(fusion_first_value_task.name, fusion_first_value_task)
...
Registry.add_task(fusion_union_task.name, fusion_union_task)
...
def _aggregate_metrics(curves: Sequence[RunCurve]) -> AggregatedMetrics:
...
def _format_metrics_table(rows: Sequence[AggregatedMetrics]) -> List[List[str]]:
    header = ["Strategy", "n", "Q best", "Evals to 95%", "Evals to best", "AOC"]
    body = [
        [
            row.strategy,
            str(row.n),
...
def plot_aggregated(
    ...
    *,
    ...
    for strategy in sorted(by_strategy):
        ...
        (line,) = ax.plot(xs, means, marker="o", markersize=3, linewidth=1.8, label=f"{strategy} (n={n})")
    ...
    ax.set_ylabel("Best quality score so far (mean)")
    ax.set_title(f"{title}\n(shaded: {band_label} across seeds)")
...
def _process_config_group(
    ...
    *,
    ...
    metrics = [_aggregate_metrics(curves) for _, curves in sorted(by_strategy.items())]
    ...
    print()
    print(f"[{config}] seeds={[s for s, _ in seed_dirs]}")

...
Full exhaustive run (all task/parameter permutations):
    python execute.py ... --configs exhaustive
"""
...
def _write_plan_snapshot(plan_path: Path, plan: KgPipePlan) -> None:
...
def _write_eval_snapshot(eval_path: Path, evaluation: Dict[str, Any]) -> None:
...
def _load_cached_eval(eval_path: Path) -> Optional[Dict[str, Any]]:
    ...
    if payload.get("status") == "error":
        return {
            "status": "error",
            "error": payload.get("error", "cached error"),
            "evaluation": None,
            "score": 0.0,
        }
...
def run_rdf_pipeline(
    ...
    *,
    ...
    _write_plan_snapshot(plan_path, plan)
...
def run_all_configs(
    *,
    ...
    print(f"reuse_existing: {reuse_existing}")
    ...
    for offset, pipeline_config in enumerate(selected, start=start):
        ...
        try:
            cached = _load_cached_eval(eval_path) if reuse_existing else None
            if cached is not None:
                ...
                if cached["status"] == "error":
                    ...
                    print(f"cached score: {cached['score']:.6f}")
                ...
                evaluation = _to_jsonable(aggregate_score)
                entry["evaluation"] = evaluation
                entry["cached"] = "result_only"
                _write_eval_snapshot(eval_path, evaluation)
                print(f"reused result, score: {aggregate_score.final_score:.6f}")
            else:
                run_pipeline(
                    pipeline_config,
                    seed_path=seed_path,
    ...
    print(f"\nFinished: {succeeded}/{len(run_results)} succeeded, cache_hits={cache_hits}")
    return run_results

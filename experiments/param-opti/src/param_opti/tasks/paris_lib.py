...
def paris_exchange(input_path: Path, output_path: Path, entity_matching_threshold: float, relation_matching_threshold: float):
    """
    ...
    """
    print(f"Converting Paris CSV to matching format with input_path: {input_path} and output_path: {output_path}")
    ...
    def filter_matches(matches: List[ER_Match]):

        for match in matches:
            if match.id_type == "entity" and match.score > entity_matching_threshold:
                yield match
            if match.id_type == "relation" and match.score > relation_matching_threshold:
                yield match

    if last_eqv_it == -1:
        doc = ER_Document(matches=list(filter_matches([])))
        ...
        doc = ER_Document(matches=list(filter_matches(matches)))
